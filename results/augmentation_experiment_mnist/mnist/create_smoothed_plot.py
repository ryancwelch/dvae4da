...
def loess_smooth(x, y, frac=0.3):
    ...
    if len(x_array) >= 4:  # Need reasonable number of points for LOESS
        ...
        smoothed = lowess(y_array, x_array, frac=frac, it=1, return_sorted=True)
        x_smooth = smoothed[:, 0]
        y_smooth = smoothed[:, 1]
    else:
...
def main():
    ...
    dataset_sizes.sort()
    print(f"Found dataset sizes (excluding size 10): {dataset_sizes}")
    ...
    # Plot lines - smooth only noisy and vae_recon
    for i, (name, acc_list) in enumerate(accuracies.items()):
        if len(acc_list) > 0:
            # Sort data points by x value
            sorted_indices = np.argsort(size_data[name])
            x_sorted = np.array(size_data[name])[sorted_indices]
            y_sorted = np.array(acc_list)[sorted_indices]
            
            if name in ['1_noisy', '2_vae_recon']:
                # Apply smoothing only to noisy and vae_recon datasets
                x_smooth, y_smooth = loess_smooth(x_sorted, y_sorted, frac=0.3)
                ...
                # For other datasets, plot direct connections between points
    ...
    plt.title('Effect of Augmentation Strategies on Downstream Classification Accuracy (MNIST)', fontsize=20)
