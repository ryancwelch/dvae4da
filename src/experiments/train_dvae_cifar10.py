...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a DVAE model on CIFAR-10 dataset")
    ...
    parser.add_argument("--noise-type", type=str, default="gaussian", 
                        choices=["gaussian", "salt_pepper", "blur", "motion_blur", "jpeg"], 
                        help="Type of noise to add")
    parser.add_argument("--noise-params", type=str, default="0.1", 
                        help="Noise parameters (comma-separated)")
    ...
    # Output parameters
    parser.add_argument("--save-dir", type=str, default="results/dvae_cifar10", 
...
def main():
    ...
    noise_params = [float(param) for param in args.noise_params.split(",")]
    ...
    trainer = Trainer(
        ...
        model_save_path=os.path.join(args.save_dir, "dvae_cifar10_model.pt")
    )
    ...
    # Save loss plots
    trainer.plot_losses()
    plt.savefig(os.path.join(args.save_dir, "dvae_cifar10_losses.png"))
    ...
    trainer.generate_reconstructions(test_loader, num_samples=10, 
                                    save_path=os.path.join(args.save_dir, "dvae_cifar10_reconstructions.png"))
    
    # Generate random samples
    print("Generating random samples...")
    trainer.generate_samples(num_samples=100, grid_size=(10, 10), 
                            save_path=os.path.join(args.save_dir, "dvae_cifar10_samples.png"))
